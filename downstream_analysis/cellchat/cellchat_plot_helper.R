...
netAnalysis_signalingRole_heatmap_modified <- function(object, signaling = NULL, pattern = c("outgoing", "incoming","all"), slot.name = "netP",
  ...
  if (!is.null(signaling)) {
    ...
    if (!is.null(targets.use)){
        mat <- mat[, targets.use, drop = FALSE]
        }
  ...
  if (min(mat, na.rm = T) == max(mat, na.rm = T)) {
    ...
    # mat <- t(mat)
    # mat.ori <- t(mat.ori)
    # row_annotation <- col_annotation  # reusing existing col_annotation
    # # col_annotation <- ha1  # the barplot annotation becomes the column annotation now
